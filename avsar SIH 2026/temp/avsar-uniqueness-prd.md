...
**Companion plan:** `~/.commandcode/plans/avsar-uniqueness-pillars.md` (written and executed; mirrored with per-phase status in `.scratch/uniqueness-pillars/` as `PLAN.md` plus `issues/01` through `issues/07`)

---

> **Status: implemented.** All five pillars are built and shipped, in both lanes. Four
> claims in the body below were wrong when this was written, and each is marked
> **[corrected]** where it appears:
>
> - `taxonomy.js` holds **62** skills, not 78.
> - `demandWeight` was never wired to anything. The market-aware term is a **new scoring
>   dimension** (`src/lib/market.js`), not a hook that needed switching on.
> - The data spine this spec describes was incomplete: the only real ISO dates actually
>   arriving were being thrown away in `src/lib/store.js:292-336` (Remotive
>   `publication_date`, Arbeitnow `created_at`).
> - `isEvidenceUrl` lives in `src/lib/quests.js`, not `proof.js`, and `crypto.subtle` is
>   available in Node 19+, so the tests exercise the real signature path.
>
> Two live bugs this spec also missed are fixed: the Ashby `secondaryLocations` join that
> produced `"loc": "[object Object]"`, and a board seed that was 49 postings from 2
> companies with Lever returning zero. The seed is now 216 postings from 16 companies.
>
> **Deliberately not done:** the ayush lane still carries no dated postings. No ayush
> scraper exists in this repo, and inventing dates would have made every trend claim in
> the product a fiction. That lane reports zero dated postings and its trend column says
> `no signal yet`.
>
> **Verification as shipped:** 412 unit tests, `npm run lint` clean, `npm run build`
> clean, and `npm run verify:routes` rendering every student route in both lanes plus all
> three desks in a real browser.
...
1. **A demand-weight hook that was never implemented.**
   `src/data/taxonomy.js` line 34 documents: `demandWeight: editable multiplier; postings nudge it via match.js recompute`. **[corrected]** That recompute did not exist, and `demandWeight` was not a half-built hook waiting to be connected: nothing read it but a metadata field on matched skills. The market term had to be built as a new scoring dimension, in `src/lib/market.js`, with an explicit `weights` argument threaded through `matchScore`. Separately, `scripts/boards.mjs` already fetches posting dates from three ATS APIs (Greenhouse `updated_at`, Ashby `publishedAt`, Lever `createdAt`) and discards all of them. **[corrected]** Those were not the only dates being lost: `src/lib/store.js:292-336` was throwing away Remotive `publication_date` and Arbeitnow `created_at`, the only real ISO dates the app received, and this list did not mention it.

2. **A skill graph that is never traversed as a graph.**
   `taxonomy.js` holds **[corrected]** 62 skills, each with `related[]` edges, `halfLifeDays` decay, `demandWeight`, and an NSQF level. Nothing in the app walks those edges. There is no pathfinding, no shortest route, no simulation. The edges turn out to form a graph with genuinely disconnected clusters, so a route has to express "no path from what you hold" as a real state rather than a defensive branch.
...
| PS-4 | Submission requires a valid evidence URL, reusing `isEvidenceUrl` (`src/lib/quests.js`, **[corrected]** not `proof.js`) | Must |
