// Ayush home: the board the vaidya portal lands on. Built as a notice board:
// a flat emerald header with the greeting, light tiles for the four numbers,
// an amber notice while the loop is open, proof of work, then today's rounds
// as three cards (the board pins one card per round; the tech console lists
// its moves instead, which is how the two homes stop looking the same).
// Home.jsx picks this component when the track is ayush.
// Dial ENERGY 1 / RHYTHM 2 / MOTION 1: static, hover feedback only.
import CIcon from "@coreui/icons-react";
import {
  cilChatBubble, cilCheckCircle, cilArrowRight,
...
// One round card header: mono stage numeral plus title. Same numeral voice as
// the tech docket, card body because the board pins whole notices, not rows.
function RoundHead({ n, title }) {
  return (
    <div className="mb-2 flex items-center gap-2">
      <span className="font-mono text-xs font-medium tabular-nums text-stone-400" aria-hidden>{n}</span>
      <H2 className="mb-0" tone="light">{title}</H2>
    </div>
  );
}
...
export default function AyushHome() {
  ...
  return (
    ...
    >
      {/* header board: flat emerald, white ink. Flat because the greeting is
          institutional, not decorative; the ring stays because readiness is
          the number this portal checks daily. */}
      <section className="rounded-xl bg-emerald-900 px-5 py-6 text-white sm:px-7">
      ...
      {/* tiles: light panels because the ink is stone. tone="light" survives
          the ayush-dark remap, so both themes hold contrast. */}
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {stats.map((s) => (
          <Card key={s.label} tone="light" className="flex items-center gap-3 p-4">
      ...
      {loop.total < 100 && (
        <Card tone="light" className="mt-4 flex flex-wrap items-center gap-3 border-amber-300 bg-amber-50">
      ...
      {/* today's rounds: pinned cards, one per round. */}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card tone="light" className="flex h-full flex-col">
          ...
          <div className="mt-auto pt-3"><Btn to="/jobs" variant="quiet" size="sm">All internships and jobs <CIcon icon={cilArrowRight} width={14} height={14} /></Btn></div>
        </Card>

        <Card tone="light" className="flex h-full flex-col">
          <RoundHead n="03" title="Ask" />
