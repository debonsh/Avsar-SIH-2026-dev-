...
// Slice C: 20-Q ayush bank, 10 sampled per run seeded by C2C ID + day.
import { test } from "node:test";
import assert from "node:assert/strict";
import { QUIZ, gradeQuiz, gradeSet, quizSample, loadQuizBest } from "../src/data/quiz.js";

test("banks exist for the ayush track, 20 Qs each, 4 opts, ans in range", () => {
  for (const role of ["ayush"]) {
...
test("quizSample: 10/20, stable per id+day, spread across ids", () => {
  const a1 = quizSample("ayush", "C2C-AAAAAA", "2026-09-30");
  const a2 = quizSample("ayush", "C2C-AAAAAA", "2026-09-30");
  ...
  const seen = new Set(
    Array.from({ length: 12 }, (_, i) => quizSample("ayush", `C2C-ID${i}`, "2026-09-30").map((item) => item.q).join("|"))
  );
