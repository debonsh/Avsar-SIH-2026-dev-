...
import { combineScores } from "../src/lib/scores.js";
...
test("COHORT: 24 students, 6 per role, required fields", () => {
  assert.equal(COHORT.length, 24);
  for (const role of ["sde", "data", "marketing", "govt"]) {
    assert.equal(COHORT.filter((s) => s.role === role).length, 6, `${role} needs 6 students`);
  }
