...
export function buildPrompt(actionId, s = {}) {
  const c = ctx(s);
  const head = c.ayush
    ? `You are a placement coach for a ${AYUSH_WHO} targeting "${c.roleLabel || "an ayush role"}". Current resume score: ${c.score ?? 0}/95. Skills ON their resume: ${c.found.join(", ") || "none listed yet"}. Top missing skills: ${c.missing.join(", ") || "none"}. Speak in clinical terms (OPD/IPD, case sheets, panchakarma, SHISHIKSHA, NCISM norms).`
    : `You are a placement coach for a Tier-2/3 Indian college student targeting "${c.roleLabel || "a tech role"}". Current resume score: ${c.score ?? 0}/95. Skills ON their resume: ${c.found.join(", ") || "none listed yet"}. Top missing skills: ${c.missing.join(", ") || "none"}. Best-fit track: ${c.bestFitLabel || c.roleLabel || "undecided"}.`;
  switch (actionId) {
    case "gaps":
