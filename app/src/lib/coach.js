...
function ctx(s = {}) {
  ...
  return { ...s, missing, top: missing[0] || "" };
}

export function buildPrompt(actionId, s = {}) {
  const c = ctx(s);
  const head = `You are a placement coach for a Tier-2/3 Indian college student targeting "${c.roleLabel || "a tech role"}". Current ATS: ${c.score ?? 0}/95. Top missing skills: ${c.missing.join(", ") || "none"}. Best-fit track: ${c.bestFitLabel || c.roleLabel || "undecided"}.`;
  switch (actionId) {
    case "gaps":
...
export function localAnswer(actionId, s = {}) {
  ...
  return "Free-text needs the AI key (add VITE_GEMINI_KEY to app/.env) — meanwhile the 4 quick actions above all work offline.";
}
