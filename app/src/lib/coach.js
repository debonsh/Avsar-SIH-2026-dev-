...
export const COACH_ACTIONS = [
  ...
  { id: "match", label: "Match me to a job" },
...
export function matchWriteup(found = [], job = null) {
  ...
  if (!m) return "Score your resume first — then I'll match it against your best eligible job.";
  return `Match: ${m.score}/100 (${m.band}) — ${job.title} at ${job.company}.\nOverlap: ${m.overlap.join(", ") || "none yet"}.\nMissing: ${m.missing.join(", ") || "none — apply now"}.\nTip: close "${m.missing[0] || "nothing"}" with one quest pair and re-match.`;
}
