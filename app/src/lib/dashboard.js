...
// deterministic AI-briefing lines (JobSync's "what matters today" without a model call)
export function briefing({ funnel = {}, jobs = [], found = [], missing = [], mainScore = 0 } = {}) {
  ...
  if (strong.length) lines.push(`${strong.length} saved-or-open role${strong.length > 1 ? "s" : ""} match 75%+ of your skills — apply first: ${strong.slice(0, 2).map((j) => j.title).join("; ")}.`);
  ...
  if ((funnel.interview || 0) > 0) lines.push(`${funnel.interview} interview${funnel.interview > 1 ? "s" : ""} in flight — prep STAR answers with numbers this week.`);
