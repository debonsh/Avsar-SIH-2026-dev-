...
export async function genInterviewQs(resumeText, role = "ayush") {
  const prompt = `You write interview questions for a BAMS (ayurveda) student targeting "${role}".
${ctx(resumeText, role)}
Write 5 questions PERSONALIZED to their resume: 2 clinical deep-dives, 2 role-knowledge, 1 behavioral. Short, spoken-style.
Return ONLY a JSON array, no prose, no fences: [{"text": "...", "dimension": "skill"}].`;
  const raw = await memoCall("aigen-interview", `${role}|${String(resumeText || "").slice(0, 1500)}`, () => chat(prompt, "interview"));
