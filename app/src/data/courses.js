...
export function resumeTips(result = {}) {
  ...
  if (has("Skills Match", /capped at/)) tips.push("Skills capped by proof volume — add 1 project with a link instead of more keywords.");
  if (has("Keyword Signal", /repetition capped/)) tips.push("Keyword repetition detected — cut repeats, add 1 quantified outcome.");
  if (has("Project Quality", /0 quantified/)) tips.push("No numbers on your resume — add 2-3 quantified outcomes (%, time, users).");
  if (has("Sections & Recency", /no dates/)) tips.push("No dates found — add years to experience and education.");
  if (has("Format & Contact", /no contact/)) tips.push("Contact block incomplete — email + phone + GitHub/LinkedIn on line 1.");
  if ((result.total ?? 0) > 0 && (result.total ?? 0) < 45) tips.push("Foundation stage — 1 project + 1 free cert moves this fastest.");
