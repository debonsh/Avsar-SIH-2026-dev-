...
const BLANK_TYPS = ["Internship", "Full-time", "Govt"];
const AYUSH_TYPS = ["Internship", "Full-time", "Govt", "ministry", "research", "training"];

const blank = { title: "", company: "", role: "sde", loc: "Remote", type: "Internship", skills: "", minScore: 40, apply: "", description: "" };
...
export default function Industry() {
  ...
  const isAyush = f.role === "ayush";

  return (
    <Page
      title="Industry"
      sub="post internships, projects, and entry-level roles with required skills. students whose scores clear your bar see them as eligible."
      ...
      <Card>
        ...
        <form onSubmit={post} className="grid gap-3 sm:grid-cols-2">
          <Field label="role title">
            <input className={inputCls} value={f.title} onChange={set("title")} placeholder="frontend intern" />
          </Field>
          <Field label="company">
            <input className={inputCls} value={f.company} onChange={set("company")} placeholder="zeta" />
          </Field>
          <Field label="track">
            <select className={inputCls} value={f.role} onChange={set("role")}>
              ...
              <option value="ayush">{AYUSH_ROLE.label}</option>
            </select>
          </Field>
          <Field label="type">
            <select className={inputCls} value={f.type} onChange={set("type")}>
              {(isAyush ? AYUSH_TYPS : BLANK_TYPS).map((t) => (
                <option key={t} value={t}>{t}</option>
              ))}
            </select>
          </Field>
          <Field label="location">
            <input className={inputCls} value={f.loc} onChange={set("loc")} placeholder="remote / bangalore" />
          </Field>
          ...
          <Field label="required skills" hint="comma separated. matching is literal against resume skills.">
            <input className={inputCls} value={f.skills} onChange={set("skills")} placeholder="react, rest apis, sql" />
          </Field>
