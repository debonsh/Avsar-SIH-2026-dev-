// AyushSetu landing: BAMS student first. Hero, Ananya story, feature cards.
// Landing components live in components/landing; primitives in ui.jsx.
import { useState } from "react";
import { Link } from "react-router";
import { ArrowRight, Leaf } from "lucide-react";
import { Btn, Card, Chip, Reveal, VaidyaLevel } from "../components/ui.jsx";
...
// Ananya story from the winning strategy (SIH 26044 §9)
const ANANYA = {
  name: "Ananya",
  year: "BAMS final year",
  target: "Clinical Research Associate — Ayush",
  assessment: "scored 44. missing gcp documentation. strong in dravyaguna.",
  roadmap: "2 certs + 1 workshop",
  fit: "87%",
  outcome: "cleared shishiksha, applied at 87% fit, mentor signed off, pharmacovigilance flipped to proof",
};
...
export default function Home() {
  ...
  return (
    <div>
      ...
      {/* Ayush banner */}
      <section className="mx-auto w-full max-w-5xl px-4 pt-8 sm:px-6" aria-label="Ministry banner">
        <div className="border border-emerald-900/60 bg-emerald-950/40 px-4 py-3 sm:flex sm:items-center sm:gap-4">
          <Leaf className="size-5 shrink-0 text-emerald-400" aria-hidden />
          <p className="text-sm text-emerald-200">
            <strong className="font-semibold text-emerald-100">Ministry of Ayush</strong> · BAMS to Vaidya pathway · NCISM aligned · SIH 26044
          </p>
          <Link to="/ayush" className="mt-2 inline-block font-mono text-xs text-emerald-400 underline underline-offset-4 hover:text-emerald-200 sm:mt-0">
            enter as bams student →
          </Link>
        </div>
      </section>
      ...
      {/* Ananya demo story */}
      <section className="mx-auto w-full max-w-5xl px-4 py-10 sm:px-6" aria-label="Ananya demo story">
        <Reveal>
          <Card className="border-emerald-900/40 bg-emerald-950/20">
            <p className="font-mono text-[11px] uppercase tracking-widest text-emerald-400">// demo story · ananya · bams final year</p>
            <h2 className="mt-3 text-balance font-display text-2xl font-bold text-zinc-50 sm:text-3xl">
              From {ANANYA.year} to {ANANYA.target}
            </h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <div className="border border-zinc-800 bg-zinc-950 px-4 py-3">
                <p className="font-mono text-[11px] uppercase tracking-widest text-zinc-500">assessment</p>
                <p className="mt-1 text-sm text-zinc-200">{ANANYA.assessment}</p>
              </div>
              <div className="border border-zinc-800 bg-zinc-950 px-4 py-3">
                <p className="font-mono text-[11px] uppercase tracking-widest text-zinc-500">roadmap</p>
                <p className="mt-1 text-sm text-zinc-200">{ANANYA.roadmap}</p>
              </div>
              <div className="border border-zinc-800 bg-zinc-950 px-4 py-3">
                <p className="font-mono text-[11px] uppercase tracking-widest text-zinc-500">match</p>
                <p className="mt-1 text-sm text-zinc-200">{ANANYA.fit} fit</p>
              </div>
            </div>
            <p className="mt-4 text-sm leading-6 text-emerald-300">{ANANYA.outcome}</p>
            <div className="mt-5 flex flex-wrap gap-3">
              <Btn to="/ayush" size="lg" className="rounded-none">
                <Leaf className="size-4" aria-hidden /> run her path
              </Btn>
              <Link to="/" className="inline-flex items-center font-mono text-xs text-zinc-500 underline underline-offset-4 hover:text-zinc-200">
                main door →
              </Link>
            </div>
          </Card>
