  ...
  return (
    <span className="inline-flex items-center gap-1" title={`Mastery ${level} of 3`}>
      {[1, 2, 3].map((i) => (
        <span
          key={i}
          className={`size-1.5 rounded-none ${i <= level ? "bg-blurple" : "bg-zinc-800"}`}
          aria-hidden
        />
      ))}
...
export default function Quests() {
  ...
  return (
    <Page
      ...
      sub={isAyush ? "Shishiksha → rotatory → proof. Check things off — the bar moves with you." : "Week 1 unlocks the most roles, so start there. Check things off — the bar moves with you."}
    >
      <Card className="mb-4">
